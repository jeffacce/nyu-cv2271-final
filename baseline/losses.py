...
https://github.com/assassint2017/MICCAI-LITS2017/tree/master/loss
Loss functions
"""
...
class HybridLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        # Definition of dice coefficient
        dice = 2 * (pred * target).sum(dim=1).sum(dim=1).sum(dim=1) / (pred.pow(2).sum(dim=1).sum(dim=1).sum(dim=1) + target.pow(2).sum(dim=1).sum(dim=1).sum(dim=1) + smooth)
        # Returns the dice distance + 　 binarized cross entropy loss
...
class JaccardLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        # Return is the jaccard distance
        return torch.clamp((1 - dice).mean(), 0, 1)
...
class MultiTverskyLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        input_slices = torch.split(inputs, [1] * num_class, dim=1) # slices the input tensor into the number of labels
